...
var GameBoard = React.createClass({displayName: "GameBoard",
	...
	onCardFlip: function(cardIndex) {
		...
		for (var i = 0; i < currentImagesFlipped.length; i += 1) {
			...
			} else if (currentImagesFlipped[cardIndex] !== undefined) {
				this.setState({
					flipCount: this.state.flipped + 1,
				})
			}
		};
	...
	render: function() {
		return React.createElement("div", {className: "game-container"}, 
				React.createElement("div", {className: "game-info"}, this.state.gameInfo, this.state.FlipCount, this.state.imagesFlipped[2]
				), 
