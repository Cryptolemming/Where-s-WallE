...
var GameBoard = React.createClass({displayName: "GameBoard",
	...
	boardCreation: function() {
		var board = [];
		for (var i = 0; i < 6; i+=1) {
			board.push(React.createElement(Card, {key: i, image: this.imagesArray[i], onClick: this.onCardFlip, flipped: this.flipped[i], cardIndex: i}));
		}; 
		return board;
	},
...
React.render(React.createElement(GameBoard, {initialState: flipped = this.flipped,
			counter = 0,
			imagesArray = this.imagesArray(),
			board = this.boardCreation(),
			flippedImages = [],
			walleCount = 0}), document.getElementById('game-board-container'));
