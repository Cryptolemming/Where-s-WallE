...
var Block = React.createClass({displayName: "Block",
	...
	render: function() {
		...
		var flip = this.state.flipped ? this.teststyle : styleDefault;
		return (
...
var GameBoard = React.createClass({displayName: "GameBoard",
	...
	render: function() {
		var teststyle = {
			backgroundImage: this.state.images.splice(Math.floor(Math.random() * this.state.images.length), 1),
		}
		var board = [];
		for (var i = 0; i < 6; i+=1) {
			board.push(React.createElement(Block, {key: i, style: teststyle}));
		}
