...
var GameBoard = React.createClass({displayName: "GameBoard",
	...
	onStartClick: function(gameOver) {
		return this.newGame();
	},
	onCardFlip: function(cardIndex) {
		var walleReduce = this.state.imagesFlipped.reduce(function(n, val) { return n + (val == 'walle.jpg') }, 0);
		if (walleReduce >= 2 || (walleReduce = 1 && this.state.imagesFlipped.length == 4) || (walleReduce == 0 && this.state.imagesFlipped.length == 3)) {
			this.setState({
				gameOver: true,
			})
		} else {
