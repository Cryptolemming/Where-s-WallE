...
var GameBoard = React.createClass({displayName: "GameBoard",
	...
	boardCreation: function() {
		...
		for (var i = 0; i < 6; i+=1) {
			board.push(React.createElement(Card, {key: i, image: this.shuffledImages[i], onClick: this.onCardFlip, flipped: flipped[i], cardIndex: i}));
		}; 
		return board;
	},
	...
	render: function() {
		...
		return React.createElement("div", {className: "game-board"}, React.createElement("ul", {className: "grid"}, this.state.board), this.state.flippedImages, this.state.walleCount);
	}
});
